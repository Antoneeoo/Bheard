...
import { NUM_BUBBLE_COLORS, NEIGHBOR_OFFSETS } from "../constants/Game";
...
export default function PlayGame() {
    ...
    // Load ball image assets
    function loadBallImages() {
        // Initialize variables
        loadcount = 0;
        loadtotal = NUM_BUBBLE_COLORS;
        preloaded = false;

        // Load the images
        var loadedimages = [];
        for (var i = 0; i < NUM_BUBBLE_COLORS; i++) {
    ...
    function getNeighbors(tile) {
        ...
        var n = NEIGHBOR_OFFSETS[tilerow];
    ...
    function drawBubble(x, y, index) {
        if (index < 0 || index >= NUM_BUBBLE_COLORS)
            return;

        // Draw the bubble sprite
        context.drawImage(bubbleimages[index], 0, 0, 60, 60, x, y, level.tilewidth, level.tileheight);
    }
    ...
    function createLevel() {
        ...
        for (var j = 0; j < level.rows; j++) {
            var randomtile = randRange(0, NUM_BUBBLE_COLORS - 1);
            var count = 0;
            for (var i = 0; i < level.columns; i++) {
                if (count >= 2) {
                    // Change the random tile
                    var newtile = randRange(0, NUM_BUBBLE_COLORS - 1);

                    // Make sure the new tile is different from the previous tile
                    if (newtile == randomtile) {
                        newtile = (newtile + 1) % NUM_BUBBLE_COLORS;
                    }
                    randomtile = newtile;
                    count = 0;
                }
